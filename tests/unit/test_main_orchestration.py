...
class TestMainInitialization:
    ...
    def test_initialize_vectordb_failure(self, mock_client):
        ...
        with patch('src.main.print') as mock_print:
            ...
            assert result is False
            mock_print.assert_any_call("❌ ChromaDB connection failed: Vectordb failure")
    ...
    @patch('src.main.initialize_user_memory')
    @patch('sys.exit')
    def test_initialize_application_vdb_failure_exits(self, mock_exit, mock_mem, mock_vdb, mock_llm):
...
class TestChatLoopErrorHandling:
    ...
    def test_main_loop_connection_error(self, mock_save, mock_logger, mock_print, mock_input, mock_init):
    ...
    @patch('src.main.print')
    @patch('src.main.save_memory')
    def test_main_loop_keyboard_interrupt(self, mock_save, mock_print, mock_input, mock_init):
